...
public class GameBoard {
    ...
    DList blackChips;
    Chip[][] chips;
    DList allConnections = new DList();
    ...
    */
    ...
    public int EvalFunc(int color) {
        /*
          ...
          if (colorOfChip == color) {
          numOfPairs = numOfPairs + factor * this.findPairs(c).length();
          } else if (colorOfChip == opColor) {
          opNumOfPairs = opNumOfPairs + factor * this.findPairs(c).length();
          }
          }
          }

          int result = (numOfPairs - opNumOfPairs)/4;
        */
        int result = 0;
	return result;
    }
    ...
    public DList findNetwork(int color) {
        ...
        DListNode current;
        if (color == 0) {
            if (blackChips.length() < 6) return allConnections;
            current = (DListNode)blackChips.front(); 
        } else {
            if (whiteChips.length() < 6) return allConnections;
            current = (DListNode)whiteChips.front(); 
        }
        ...
        if (goalChips.isEmpty()) return allConnections;

        try {
            DListNode n = (DListNode)goalChips.front();
            while (n.isValidNode()) {
                Chip c = new Chip(((Chip)n.item()).xcorr, ((Chip)n.item()).ycorr);
                c.whatColor = ((Chip)n.item()).whatColor;
                c.isAvailable = false;
                DList lst = new DList();
                lst.insertBack(c);
                recursiveConnect(lst);
                n = (DListNode)n.next();
            }
        } catch (InvalidNodeException e) { }

        if (allConnections.length() == 0) return allConnections;
        else {
            try {
            DListNode curr = (DListNode)allConnections.front();
            while (curr.isValidNode()) {
                DList testLst = (DList)curr.item();
                if (testLst.length() < 6) {
                    curr.remove();
                } else if (((Chip)testLst.back().item()).xcorr != 7 && ((Chip)testLst.back().item()).ycorr != 7) {
                    curr.remove();
                } else {
                    DListNode n1 = (DListNode)testLst.front();
                    DListNode n2 = (DListNode)testLst.front().next();
                    DListNode n3 = (DListNode)testLst.front().next().next();
                    Chip c1 = (Chip)n1.item();
                    Chip c2 = (Chip)n2.item();
                    Chip c3 = (Chip)n3.item();
                    while (n1.isValidNode() && n2.isValidNode() && n3.isValidNode()) {
                        if (this.direction(c1,c2,c3)) {
    ...
    protected void recursiveConnect(DList lst) {
        try {
        Chip c = (Chip)lst.back().item();

        //reach the end
        if (c.xcorr == 7 || c.ycorr == 7) {
            allConnections.insertBack(lst);
        }
        DList pairs = this.findPairs(c);
        DList pairsRevised = this.trimPairs(pairs);
        if (pairs.length() != 0) {
                DListNode curr = (DListNode)pairsRevised.front();
                Chip currChip = (Chip)curr.item();
                DList lst1 = new DList();
                DListNode cur = (DListNode)lst.front();
                while (cur.isValidNode()) {
                    Chip curChip = new Chip(((Chip)cur.item()).xcorr, ((Chip)cur.item()).ycorr);
                    curChip.whatColor = ((Chip)cur.item()).whatColor;
                    curChip.isAvailable = false;
                    lst1.insertBack(curChip);
                    this.recursiveConnect(lst1);
